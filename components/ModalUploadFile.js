...
export default function ModalUploadFile({
  ...
  useEffect(() => [setSelectedFile()], [tab]);

  const { mutate: mutateBulkResources } = useSubmitBulkResources({
    options: {
      onSettled: (data) => {
        refetch();
        reportUploaded(data);
      },
      enabled: tab === 'resource',
    },
  });

  const { mutate: mutateBulkResourcesCompletion } =
    useSubmitBulkResourcesCompletion({
      options: {
        onSettled: (data) => {
          refetch();
        ...
        enabled: tab === 'resource-completion',
      },
    });
    ...
    Papa.parse(file, {
      ...
      complete: (results) => {
        ...
        if (tab === 'resource') {
          ...
          payload = results.data.map((data) => ({
            title: data['Item Title'],
            employee_id: data['User ID (External) '],
            external_id: data['External Item ID'],
            email: data['Email'],
            ...
            status: data['Status'],
            source: data['Source'],
            gems: parseInt(data['Gems']),
            coins: parseInt(data['Coins']),
            scores: parseInt(data['Score']),
          }));
        }
  ...
  const SubmitFile = () => {
    ...
    axios
      ...
      .then(() => {
        setSelectedFile();
        if (tab === 'resource') {
          mutateBulkResources({ resources: data });
        } else {
          mutateBulkResourcesCompletion({ records: data });
        }
      });
  };
  ...
  const reportUploaded = (bulkData) => {
    ...
    let reportsPayload = {
      ...
      context:
        tab === 'resource'
          ? 'resource_bulk_create'
          : 'resource_completion_bulk_create',
      remarks,
      status: status,
    };
            ...
            <div className="inline-block align-bottom bg-white rounded-lg px-4 pt-5 pb-4 text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:p-10">
              <div className="sm:flex sm:items-start">
                <div className="mt-2 text-center sm:mt-0 sm:text-left">
                  ...
                  >
                    Bulk Upload{' '}
                    {tab === 'resource' ? 'Resource' : 'Resource Completion'}
                  </Dialog.Title>
                      ...
                      <li>
                        ...
                        <span>
                          {tab === 'resource' ? (
                            ...
                            >
                              Resources_Bulk_Upload_Tempate.csv
                            </a>
                          ) : (
                            <a
                              href={templateResourceCompletion}
                              className="hover:underline text-base-600"
                            >
                              Resources_Completion_Bulk_Upload_Tempate.csv
                            </a>
                          )}{' '}
                        </span>
                      </li>
