...
export default function ModalSuccesUpload({ Show, setShow, statusAndRemarks }) {
  const variants = {
    open: { opacity: 1 },
    closed: { opacity: 0 },
  };
    ...
    >
      <div className="w-max height-78 flex z-20 fixed bottom-5 left-5 bg-gray-100 rounded-xl shadow-lg p-4 ">
        ...
        <div className="width-max-280 ml-3">
          <h1 className="font-bold text-sm leading-5 text-black">{statusAndRemarks?.status === 'success' ? "Successful. Your file has been uploaded." : "Unsuccessful. Your file has failed to be uploaded."}</h1>
          <p className="text-sm leading-5 font-normal text-gray-800">{statusAndRemarks?.remarks}</p>
        </div>
        <button
