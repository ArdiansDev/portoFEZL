...
export default function ModalResource({ open, setOpen, onAddItem }) {
  ...
  useEffect(() => {
    if (resources?.data) {
      setResourceData(JSON.parse(JSON.stringify(resources.data)));
  ...
  const handleChange = (id) => {
    ...
    let newMedia = resourceData.map((itm) => {
      if (itm.id == id) {
    ...
    setResourceData(JSON.parse(JSON.stringify(newMedia)));
  };
  ...
  const tbody =
    resourceData?.map((data) => ({
      ...
      tdata: [
        ...
        {
          variant: 'dataNumeric',
          data: { title: data['rewardScore'] },
        },
        {
          variant: 'dataNumeric',
          data: { title: data['rewardCoin'] },
        },
        {
          variant: 'dataNumeric',
          data: { title: data['rewardGem'] },
        },
      ],
    })) || [];
  return (
                ...
                <div className="mt-2 text-center sm:mt-0 sm:text-left">
                  ...
                  >
                    <p className="flex">
                      ...
                      You can add multiple existing resources or create a new
                      one.
                    </p>
                    <button onClick={() => router.push('/admin/resource')}>
                      Create New Resource →
                    </button>
                  </Dialog.Title>
