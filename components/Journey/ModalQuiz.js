...
import { useQuizAdmin } from 'hooks/journeys/quiz/useQuiz';
import { useCategories } from 'hooks/categories/useCategories';
import Filter from 'components/Filter/Filter';
import router from 'next/router';

export default function ModalQuiz({ open, setOpen, data, onAddItem }) {
  ...
  useEffect(() => {
    if (quizzes?.data) {
      setQuizData(quizzes);
      let totalPages = quizzes?.pagination?.totalPages || 1;
  ...
  const handleChange = (id) => {
    ...
    let newLink = quizData?.data.map((itm) => {
      if (itm.id == id) {
        itm.checked = itm.checked ? false : true;
      }
      return itm;
    });
    quizData['data'] = newLink;
    ...
    setQuizData(JSON.parse(JSON.stringify(quizData)));
  };
  ...
  const tbody =
    quizData?.data?.map((item) => ({
      ...
      tdata: [
        ...
        { variant: 'data', data: { title: item.title } },
        { variant: 'dataNumeric', data: { title: item.rewardCoin } },
        { variant: 'dataNumeric', data: { title: item.rewardGem } },
        { variant: 'dataNumeric', data: { title: item.rewardScore } },
      ],
    })) || [];
  return (
