              ...
              {step.status === 'complete' ? (
                <a href={step.href} className="group">
                  <span className="flex items-start">
                    <span className="flex-shrink-0 relative h-5 w-5 flex items-center justify-center">
                      <CheckCircleIcon
                        className="h-full w-full text-base-600 group-hover:text-base-800"
                        aria-hidden="true"
                      />
                    </span>
                ...
                <a href={step.href} className="flex items-start" aria-current="step">
                  <span className="flex-shrink-0 h-5 w-5 relative flex items-center justify-center" aria-hidden="true">
                    <span className="absolute h-4 w-4 rounded-full bg-base-200" />
                    <span className="relative block w-2 h-2 bg-base-600 rounded-full" />
                  </span>
                  <span className="ml-3 text-sm font-medium text-base-600">{step.name}</span>
                </a>
              ) : (
                <a href={step.href} className="group">
