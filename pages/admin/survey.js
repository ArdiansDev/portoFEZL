...
export default function Survey() {
  ...
  const tbody = surveys?.data?.map((survey) => ({
    key: survey.id,
    tdata: [
      { variant: 'data', data: { title: survey['name'] } },
      {
        variant: 'dataNumeric',
        data: { title: survey['reward_score'] },
      },
      {
        variant: 'dataNumeric',
        data: { title: survey['reward_coin'] },
      },
      {
        variant: 'dataNumeric',
        data: { title: survey['reward_gem'] },
      },
    ],
  }));
  ...
  return (
    <div className="flex-col ">
      ...
      <h1 className="text-xl my-4 font-medium leading-7 text-black">Survey</h1>{' '}
        <div className="flex items-center py-5 gap-11">
          <SearchForm onSearch={onSearch} placeholder={'Search by Title'} />

          <button
            class=" hidden width-195 bg-blue-900 height-35 hover:bg-grey  py-2 px-4 rounded inline-flex items-center"
            onClick={() => {
              setUploadModal(true);
            }}
          >
