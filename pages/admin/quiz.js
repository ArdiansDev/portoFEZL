...
export default function Quiz() {
  ...
  useEffect(() => {
    if (quiz?.data) {
      let totalPages = quiz?.pagination?.totalPages || 1;
  ...
  const tbody = quiz?.data?.map((data) => ({
    key: data.id,
    tdata: [
  ...
  return (
    <div className="flex-col ">
      <section className=" mobile:ml-0 mobile:mt-4 w-full ">
        <h1 className="text-xl my-4 font-medium leading-7 text-black">Quiz</h1>{' '}
